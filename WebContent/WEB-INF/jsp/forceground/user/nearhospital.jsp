	...
	<div style="width:100%;height:100%;position: absolute;">
		...
		<div style="width:100%;height:130%;">
			...
			<div style="width:70%;height:100%;float:left;">
					...
					<div style="width:100%;height:50%">
						...
						<div>
							<table class="table table-hover" style="padding-top:5px">
								...
								<tbody id="id_hospital_list"></tbody>
							</table>
						</div>
					</div>
					<div style="width:100%;height:50%">
						...
						<div>
							<table class="table table-hover" style="padding-top:5px">
								...
								<tbody id="id_bloodlib_list"></tbody>
							</table>
						</div>
					</div>
				</div>
				...
				<div style="width:10%;height:100%;float:left">医院详情</div>
			</div>
			<!-- 右侧空白 -->
			<div style="width:15%;height:100%;float:left;background-color: #DDDDDE"></div>
		</div><!-- 中间主体div结束 -->
...
function loadHospitalTable(){
	$('#id_hospital_list').empty();
	$.when($.get('${pageContext.request.contextPath}/app/user/getnear/2')).done(function(data){
		rows=data.rows;
		var li='';
		for(var i=0;i<rows.length;i++){
			...
			li+='<td><button class="btn btn-danger btn-sm" onclick="openHospital('+rows[i].userId+')">进入</button></td>';
			li+='</tr>';
		}
		$('#id_hospital_list').append(li);
	});
}
//加载血库表格
function loadBloodLibTable(){
	$('#id_bloodlib_list').empty();
	$.when($.get('${pageContext.request.contextPath}/app/user/getnear/3')).done(function(data){
		...
		for(var i=0;i<rows.length;i++){
			...
			li+='<td>'+rows[i].userName+'</td>';
			li+='<td>'+rows[i].homeAddress+'</td>';
			li+='<td><button class="btn btn-danger btn-sm" onclick="openBloodLib('+rows[i].userId+')">进入</button></td>';
			li+='</tr>';
		}
		$('#id_bloodlib_list').append(li);
	});
}

//打开医院详情页
function openHospital(aimId){
	window.location.href="${pageContext.request.contextPath}/app/fgnavi/showneardetail/2?aimId="+aimId;
}
//打开血库详情页
function openBloodLib(aimId){
