...
function loadNBTable(){
	...
	$.when($.get('${pageContext.request.contextPath}/app/needblood/getmynblistwithorder')).done(function(data){
		...
		if("0"==rows[i].isreceived){
			...
			li+='<td><button type="button" class="btn btn-primary" onclick="resolve('+rows[i].needBloodId+')">解决</button></td>'
		}else{
...
function resolve(nbid){
	layer.confirm('确定要标记为解决么',{btn:['解决','取消'],shade:0.7,shadeClose:true}
	,function(){
		$.when($.post('${pageContext.request.contextPath}/app/needblood/resolve',{needBloodId:nbid})).done(function(data){
			myalert(data);
		});
	},function(){
		layer.close();
...
function Alert(alertLevle,content){
	var left_index=(document.body.clientWidth)/3;
	var top_index=(document.body.clientHeight)/3;
	var alertHtml='<div class="alert alert-'+alertLevle+'" role="alert" style="position:fixed;top:'+top_index+';left:'+left_index+';">'+content+'</div>';
	$('#id_alert_div').append(alertHtml)
}
